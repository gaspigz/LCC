...
int barberoOcupado = 0, barberoCortando = 0, clientePagando = 0;
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutexCortando = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutexPagando = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t barberoOcupadoCond = PTHREAD_COND_INITIALIZER;
pthread_cond_t barberoCortandoCond = PTHREAD_COND_INITIALIZER;
pthread_cond_t clientePagandoCond = PTHREAD_COND_INITIALIZER;
int i = 1;
...
void* cliente(void* args){
    ...
    while(1){
        if(sem_trywait(&sillasDisponibles) == EAGAIN){
            ...
            pthread_mutex_lock(&mutexCortando);
            me_cortan(i);
            while(barberoCortando == 1){
                printf("El barbero esta cotando no puedo pagar\n");
                pthread_cond_wait(&barberoCortandoCond, &mutexCortando);
            }
            clientePagando = 1;
            pagando();
            clientePagando = 0;
            pthread_mutex_unlock(&mutexCortando);
            pthread_mutex_unlock(&mutex);
            pthread_cond_signal(&clientePagandoCond);
...
void* barbero(void* args){
    while(1){
        ...
        cortando();
        barberoCortando = 0;
        pthread_cond_signal(&barberoCortandoCond);
        pthread_mutex_lock(&mutexPagando);

        while(clientePagando == 1){
            printf("El cliente esta pagando no puedo seguir cortando\n");
            pthread_cond_wait(&clientePagandoCond, &mutexPagando);
        }
        me_pagan();
        barberoOcupado = 0;
        pthread_mutex_unlock(&mutexPagando);
