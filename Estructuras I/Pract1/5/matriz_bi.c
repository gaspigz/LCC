...
Matriz matriz_crear(){
    ...
    matriz->datos = malloc(sizeof(float*)*MULTIPLICADOR);
    for(int i = 0; i<MULTIPLICADOR; i++){
        matriz->datos[i] = malloc(sizeof(float) * MULTIPLICADOR);
    }
    return matriz;
}
