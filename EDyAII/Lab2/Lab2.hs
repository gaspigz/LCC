...
arbol1 = Node (Node (Leaf 5) (Node (Leaf 6) E)) (Node (Node (Node (Node (Leaf 10) (Leaf 12)) E) (Node (Leaf 16) (Leaf 19))) (Leaf 20))
arbol2 = Node (Node (Leaf 10) (Leaf 15)) (Node (Leaf 16) E)
arbol3 = Node (Node (Leaf 10) (Leaf 15)) (Node (Leaf 16) (Leaf 20))
...
altura E = 0
altura (Leaf a) = 1
altura (Node a b) = 1 + max2 (altura a) (altura b)
...
perfecto (Node (Node a b) E) = False
perfecto (Node E (Node a b)) = False
perfecto (Node a b) = (altura a == altura b) && perfecto a && perfecto b

-- hojaMaxProf :: Btree a -> Int -- Tecnicamente es la altura jeje
hojaMinProf :: BTree a -> Int

hojaMinProf E = 0
hojaMinProf (Leaf _) = 1
hojaMinProf (Node E E) = 1
hojaMinProf (Node a b) = min (1 + hojaMinProf a) (1 + hojaMinProf b)



perfecto2 a = hojaMinProf a == altura a
...
g2bt' (NodeG a []) (x:xs) = NodeB EB a (g2bt' x xs)
g2bt' (NodeG a (x:xs)) [] = NodeB (g2bt' x xs) a EB
g2bt' (NodeG a (x:xs)) (y:ys) = NodeB (g2bt' x xs) a (g2bt' y ys) -- xs hijos, ys hermanos
...
tieneDosHijos :: BinTree a -> Bool
tieneDosHijos EB = False
tieneDosHijos (NodeB EB a b) = False
tieneDosHijos (NodeB a b EB) = False
tieneDosHijos (NodeB a b c) = True
...
listaMasLarga [[]] = []
listaMasLarga [x] = x
listaMasLarga (x:y:xs)
   | length x >= length y = listaMasLarga (x:xs)
   | otherwise = listaMasLarga (y:xs)

dcn :: BinTree a -> [a]
dcn EB = []
dcn a = ultimoCompleto a

nivel :: Int -> BinTree a -> [a]
nivel _ EB = []
nivel 0 (NodeB _ a _) = [a]
nivel n (NodeB l _ r) = nivel (n-1) l ++ nivel (n-1) r

pot2 :: Int -> Int
pot2 0 = 1
pot2 n = 2 * pot2 (n-1)

condicion :: ([a], Int) -> Bool
condicion (xs, n) = length xs == n

ultimoCompleto :: BinTree a -> [a]
ultimoCompleto t = last (takeWhile condicion [(nivel i t, pot2 i) | i <- [0..]])
